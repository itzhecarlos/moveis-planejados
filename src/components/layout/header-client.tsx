...
import { LockKeyhole, Menu, Search, UserRound, X } from "lucide-react";
...
export function HeaderClient({ showAdminPanel }: HeaderClientProps) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-stone-200/80 bg-white/90 backdrop-blur">
      ...
      <div className={cn("lg:hidden", open ? "visible opacity-100" : "invisible opacity-0")}>
        <button
          aria-label="Fechar menu móvel"
          className="fixed inset-0 top-20 z-40 bg-black/25"
        ...
        <div className="absolute inset-x-0 top-full z-50 border-t border-stone-200 bg-white">
          <div className="container-shell space-y-4 py-4">
            ...
            <div className="grid gap-3">
              <Button className="w-full" href="/entrar" variant="secondary">
                <UserRound className="size-4" />
                Entrar na conta
              </Button>
              {showAdminPanel ? (
                <Button className="w-full" href="/admin" variant="secondary">
                  <LockKeyhole className="size-4" />
                  Entrar no painel
                </Button>
              ) : null}
