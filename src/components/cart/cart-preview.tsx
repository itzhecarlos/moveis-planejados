...
export function CartPreview({ mobile = false }: CartPreviewProps) {
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event: MouseEvent) {
      if (!mobile && !panelRef.current?.contains(event.target as Node)) {
        setOpen(false);
      }
    }
  ...
  return (
    <div className={cn("relative", mobile && "contents")} ref={panelRef}>
      <button
        aria-expanded={open}
        aria-haspopup="dialog"
        aria-label="Abrir prévia do carrinho"
        className={cn(
          "relative rounded-full border border-stone-200 p-3 hover:border-stone-300",
          mobile && "inline-flex size-12 items-center justify-center bg-white"
        )}
      ...
      {mobile ? (
        ...
        >
          <button
            ...
            type="button"
          />
          <div className="absolute inset-x-0 bottom-0 max-h-[85vh] rounded-t-[2rem] bg-white p-4 shadow-soft">
            <CartPreviewPanel
              clear={clear}
              enriched={enriched}
...
type EnrichedItem = {
  ...
  product: ReturnType<typeof getProducts>[number] | undefined;
  price: number;
  total: number;
};
...
function CartPreviewPanel({ enriched, subtotal, onClose, clear, removeItem }: CartPreviewPanelProps) {
  return (
    <>
      ...
      {enriched.length ? (
        <>
          <div className="mt-4 max-h-[50vh] space-y-3 overflow-y-auto pr-1 sm:max-h-[360px]">
          ...
          <div className="mt-4 space-y-4 border-t border-stone-100 pt-4">
            ...
            <p className="text-xs leading-6 text-stone-500">
              O valor final pode receber 5% de desconto adicional em pagamentos via Pix no checkout.
            </p>
            <div className="grid gap-3">
              <Button href="/checkout" onClick={onClose}>
                Concluir compra
              </Button>
              <div className="grid grid-cols-2 gap-3">
                <Button href="/carrinho" onClick={onClose} variant="secondary">
                  Ver carrinho
                </Button>
                <button
        ...
        <div className="py-6">
          <EmptyState description="Adicione itens para ver uma prévia rápida da sua compra por aqui." title="Carrinho vazio" />
        </div>
      )}
    </>
  );
